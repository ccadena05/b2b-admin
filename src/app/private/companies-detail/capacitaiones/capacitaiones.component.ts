...
@Component({
  selector: 'app-capacitaiones',
  templateUrl: './capacitaiones.component.html',
  styleUrls: ['./capacitaiones.component.scss']
})
export class CapacitaionesComponent implements OnInit {
  ...
  constructor(
    ...
    // this.output.image_name.subscribe(
    //   data => this.form.patchValue({ image_name: data })
    // )

    this.get()

    this.form.controls['country'].valueChanges.subscribe(
      country => this.states(country)
    )

    this.form.controls['state'].valueChanges.subscribe(
      country => this.cities(country)
    )
      ...
      (languages: any) => {
        ...
        this.provider.BD_ActionGet('general', 'get_isos').subscribe(
          (isos: any) => {
            ...
            //   (type_company: Response) => { if (!type_company.error) this.sel['type_company'] = type_company.msg }

            // )
            this.provider.BD_ActionGet('profile_company', 'get_profile', { profile_company_id: this.profile_company_id }).subscribe(
              (company: any) => {
