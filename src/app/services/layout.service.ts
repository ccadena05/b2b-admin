...
export class LayoutService {
   ...
   grid(el: HTMLFormElement | HTMLDivElement) {
   ...
   half_grid(el: HTMLFormElement | HTMLDivElement) {
   ...
   full(el: HTMLFormElement | HTMLDivElement) {
   ...
   width(el: HTMLFormElement | HTMLDivElement) {
      return el.offsetWidth;
   }

   invalid(form: FormGroup, control: string) {
      return form.controls?.[control].invalid && form.controls?.[control].touched ? 'border-2 !border-red-500 !text-red-500' : ''
   }

   label(form: FormGroup, control: string) {
      return form.value[control] ? '-top-2 text-xs' : ''
   }
